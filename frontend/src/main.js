    ...
    <div class="app-shell">
      <nav class="navbar">
        <div class="market-topbar">
          <div class="container market-topbar-inner">
            <span>Terhubung Marketplace, POS, dan LogistiKita</span>
            <div>
             d <span>Mode frontend demo</span>
              <span>Fee marketplace 2%</span>
              <span>Checkout via API Gateway</span>
            </div>
          </div>
        </div>
        <div class="container navbar-inner">
          ...
          <button class="category-menu" type="button" data-page="product-list">Kategori</button>
      ...
      <main class="container" id="app-content">
        ...
        <div id="page-product-list" class="page">
          ...
          <section class="marketplace-home-grid" aria-label="Etalase utama marketplace">
            ...
            <aside class="marketplace-side-panel">
              <h3>Ringkasan Belanja</h3>
              <p>Produk aktif, seller, dan status order memakai data dummy frontend.</p>
              <div class="side-panel-row">
                <span>Produk aktif</span>
                <strong>11</strong>
              </div>
              <div class="side-panel-row">
                <span>Seller UMKM</span>
                <strong>10</strong>
              </div>
              <button class="btn btn-primary full-width" type="button" data-focus-catalog>Cari Produk</button>
            </aside>
          </section>

          <section class="quick-category-section" aria-label="Kategori cepat">
            <div class="section-heading split-title">
              <div>
                <h2>Kategori Pilihan</h2>
                <p>Shortcut cepat seperti homepage marketplace.</p>
              </div>
            </div>
            <div class="quick-category-grid">
              <button type="button" data-category-chip="Makanan"><span>MK</span>Makanan</button>
              <button type="button" data-category-chip="Minuman"><span>MN</span>Minuman</button>
              <button type="button" data-category-chip="Fashion"><span>FS</span>Fashion</button>
              <button type="button" data-category-chip="Kerajinan"><span>KJ</span>Kerajinan</button>
              <button type="button" data-category-chip="Kesehatan"><span>KS</span>Kesehatan</button>
              <button type="button" data-category-chip="all"><span>ALL</span>Semua</button>
            </div>
          </section>
...
function renderNav() {
  ...
  if (state.currentUser) {
    ...
    links.push({ page: "cart", href: "#/cart", label: "Keranjang", match: ["/cart"], requiresLogin: true });
  ...
  nav.innerHTML = `
    ${links
      .map((link) => {
        ...
        const authAttr = link.requiresLogin ? "data-login-required" : `data-page="${link.page}"`;
        return `<a class="nav-link nav-link-${link.page} ${isActive ? "active" : ""}" href="${link.href}" ${authAttr}>${link.label}</a>`;
      })
      .join("")}
