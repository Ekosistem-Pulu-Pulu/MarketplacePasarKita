...
const statuses = ["Menunggu Pembayaran", "Pembayaran Diproses", "Pesanan Dikemas", "Pesanan Dikirim", "Pesanan Selesai"];
    ...
    <section class="container order-detail-layout">
      <main>
        <section class="order-status-card"><div class="order-status-head"><div class="status-hero-icon"><span data-lucide="${order.status === "Pesanan Selesai" ? "package-check" : "truck"}"></span></div><div><span class="status-pill ${statusClass(order.status)}">${order.status}</span><h2>${activeIndex < 3 ? "Pesananmu sedang kami siapkan" : activeIndex === 3 ? "Pesanan sedang dalam perjalanan" : "Pesanan telah selesai"}</h2><p>Setiap perubahan status akan tercatat di halaman ini.</p></div></div>
          <div class="status-timeline">${statuses.map((status, index) => `<div class="${index <= activeIndex ? "active" : ""}"><span>${index < activeIndex ? `<i data-lucide="check"></i>` : index + 1}</span><strong>${status}</strong><small>${index <= activeIndex ? (index === activeIndex ? "Status saat ini" : "Selesai") : "Menunggu"}</small></div>`).join("")}</div>
        </section>
      ...
      <aside class="summary-card order-side">
        ...
        ${order.status === "Menunggu Pembayaran" ? `<a class="btn btn-primary full" href="#/payment/${order.id}">Bayar Sekarang</a>` : activeIndex < statuses.length - 1 ? `<button class="btn btn-soft full" id="advance-status">Simulasikan Status Berikutnya</button>` : `<button class="btn btn-secondary full">Beri Ulasan</button>`}
      </aside>
    </section>
  `;
}
...
export function afterRender({ params, renderRoute }) {
  document.querySelector("#advance-status")?.addEventListener("click", () => {
    const order = getOrder(params.id);
    const nextStatus = statuses[Math.min(statuses.length - 1, statuses.indexOf(order.status) + 1)];
